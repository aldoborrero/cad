...
pkgs.rustPlatform.buildRustPackage {
  ...
  patches = [
    ./konnect-placement-clustering.patch
    # Assign explicit import identities and recover duplicate metadata identifiers.
    ./konnect-field-uuid-repair.patch
  ];
  ...
  postCheck = ''
    cargo test --offline --release \
      ...
      --package konnect-core duplicate_metadata_uuid_tests
    cargo test --offline --release \
      --target ${pkgs.stdenv.hostPlatform.rust.rustcTarget} \
      --package konnect-ipc
  '';
