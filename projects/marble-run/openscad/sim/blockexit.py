...
def drop(part="yellow", v_in=0.0, restitution=0.4, mu=0.35, seconds=3.0, stack=0):
    ...
    for t, pos, vel in core.track(ball, seconds):
        ...
        if crossed is None and pos[0] > FACE:
            ...
            dip = (
                float(np.degrees(np.arctan2(-vel[2], horiz))) if horiz > 1e-6 else 90.0
            )
            crossed = dict(
                t=t, speed=speed, dip=dip, z=pos[2], y=pos[1], entered=entered
            )
            break
    p.disconnect()
    return crossed
...
def main():
    ...
    print(
        f"{'blocks above':>13} {'e':>5} {'mu':>5} | {'in m/s':>8} {'out m/s':>8} "
    ...
    for stack in (0, 1, 2, 4, 6, 9):
        ...
        for r, pt in zip(results, points):
            if r is None:
                print(
                    f"{stack:>13} {pt['e']:5.2f} {pt['mu']:5.2f} |  never left the block"
                )
                continue
            ...
            keep = (
                100 * r["speed"] / r["entered"] if r["entered"] > 0.05 else float("nan")
            )
            print(
                f"{stack:>13} {pt['e']:5.2f} {pt['mu']:5.2f} | {r['entered'] or 0:8.2f} "
                f"{r['speed']:8.2f} {keep:5.0f}% {r['dip']:8.1f} {r['z']:7.1f}".replace(
                    "  nan%", "    - "
                )
            )
        if speeds:
            ...
            print(
                f"{'':13} {'':5} {'mean':>5} | {mi:8.2f} {np.mean(speeds):8.2f} "
