...
for m in re.finditer(
    r'\(via\s*\n?\s*\(at [\d.]+ [\d.]+\)[\s\S]{0,400}?\(net (?:\d+ )?"([^"]+)"\)', board
):
    n = m.group(1).split("/")[-1]
    if n in POWER_NETS:
        vias_per[n] = vias_per.get(n, 0) + 1
via_fail = {
    n: (vias_per.get(n, 0), floor)
    for n, floor in VIA_FLOOR.items()
    if vias_per.get(n, 0) < floor
}
...
expected_zones = (
    ["DCBUS plane (power half)", "PGND plane (power half)"]
