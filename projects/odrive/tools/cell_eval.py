...
def overlap(a, b):
    return (
        min(a[2], b[2]) - max(a[0], b[0]) > 0.01
        and min(a[3], b[3]) - max(a[1], b[1]) > 0.01
    )
...
gate = [
    (c["r"], c["q"], round(D(c["r"], c["q"]), 2))
    for c in net["gate_chains"]
    if c["r"] in pl and c["q"] in pl
]
gate_d = [g[2] for g in gate]
# driver to each gate R (drv side)
drv = [
    round(D("U3", c["r"]), 2) for c in net["gate_chains"] if c["r"] in pl and "U3" in pl
]
# commutation: per phase, min distance from any commutation cap to that phase's HI FETs
comm_caps = [
    c for c, cc in net["caps"].items() if cc["kind"] == "commutation" and c in pl
]
comm = {}
for ph in "ABC":
    his = [
        q
        for q, f in net["fets"].items()
        if f["phase"] == ph and f["role"] == "HI" and q in pl
    ]
...
sh = {
    r: round(
        min(
            D(r, q)
            for q, f in net["fets"].items()
            if f["phase"] == s["phase"] and f["role"] == "LO" and q in pl
        ),
        2,
    )
    for r, s in net["shunts"].items()
    if r in pl
}
# snubber R-C adjacency (R31/C35 A, R32/C36 B, R33/C37 C)
snub_pairs = {"A": ("R31", "C35"), "B": ("R32", "C36"), "C": ("R33", "C37")}
snub = {
    ph: round(D(a, b), 2) for ph, (a, b) in snub_pairs.items() if a in pl and b in pl
}
# area of the cell bbox
if boxes:
    xs0 = min(b[0] for b in boxes.values())
    ys0 = min(b[1] for b in boxes.values())
    xs1 = max(b[2] for b in boxes.values())
    ys1 = max(b[3] for b in boxes.values())
...
if "J2" in pl:
    ...
    for ph in "ABC":
        ...
        if qs:
            cx = sum(C(q)[0] for q in qs) / len(qs)
            cy = sum(C(q)[1] for q in qs) / len(qs)
...
J = round(
    3 * maxg
    + 2 * meang
    + max(comm.values() or [99])
    + max(sh.values() or [99])
    + max(snub.values() or [99])
    + area / 100
    + (j2p or 99) / 10
    + max(drv or [99]) / 2,
    2,
    ...
    json.dumps(
        {
            ...
            "missing": missing[:8],
            "extra": extra[:8],
            "bad_rotation": bad_rot[:8],
            "out_of_region": out_of_region[:8],
            "overlaps": overlaps[:8],
            "obstacle_hits": obst_hits[:8],
            "gate_max_mm": maxg,
            "gate_mean_mm": meang,
            "gate_worst": sorted(gate, key=lambda g: -g[2])[:3],
            "drv_to_gateR_max_mm": max(drv) if drv else None,
            "commutation_per_phase_mm": comm,
            "shunt_mm": sh,
            "snubber_rc_mm": snub,
            "area_mm2": area,
            "cell_bbox": cellbox,
            "j2_to_farthest_phase_mm": j2p,
            "J_cost": J,
        }
    )
)
