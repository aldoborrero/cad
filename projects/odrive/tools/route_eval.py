...
while True:
    ...
    for pm in re.finditer(
        r'\(pad "[^"]*" \w+ [\s\S]{0,300}?\(at ([\d.-]+) ([\d.-]+)[^)]*\)[\s\S]{0,300}?\(net (?:\d+ )?"([^"]+)"\)',
        blk,
    ):
...
# ---------- L1: lengths ----------
routed = {
    n: sum(math.hypot(x2 - x1, y2 - y1) for x1, y1, x2, y2, w, l in ss)
...
try:
    sys.path.insert(
        0,
        "/nix/store/4vbk93m9bcgga3780b04l6z3n4y5pf90-kicad-base-10.0.4/lib/python3.14/site-packages",
    )
    import pcbnew
    ...
    json.dumps(
        {
            ...
            "L1": {
                ...
                "detour_ratio_total": round(sig_routed_total / sig_lower_total, 3)
                if sig_lower_total
                else None,
                "vias_total": sum(vias_per.values()),
                "power_routed_mm": round(
                    sum(v for n, v in routed.items() if n in POWER)
                ),
            },
            "L2": {
