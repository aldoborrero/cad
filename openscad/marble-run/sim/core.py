"""The marble, the world and the sweep runner, shared by every piece's simulation.

Everything is SI; the CAD is in mm, hence MM.

    world(...)        a connected pybullet world with the solver set up
    static_mesh(stl)  the CAD's own STL as a fixed concave body
    marble(...)       the Quadrilla marble, correctly massed
    sweep(fn, **axes) one case per point of a grid, outcomes tallied
...
# Quadrilla's marble is 16 mm GLASS, not PLA: 5.36 g at 2500 kg/m3. A PLA marble would weigh
# a third of that and move nothing, which is why the seesaw's arm has to be balanced.
MARBLE_D = 16.0 * MM
MARBLE_RHO = 2500.0
MARBLE_R = MARBLE_D / 2
...
def track(ball, seconds, stop=None, dt=DT):
    """Step the world, yielding (t, position_mm, velocity) until `stop(t, pos, vel)` is True.

    Positions come back in mm, to compare against CAD dimensions directly."""
...
def build_part(part, overrides=None, obj=False, outdir="/tmp/mr-sim"):
    """Build one `part` from marble-run.scad with optional -D overrides; return the mesh path.

    Cached, so a sweep over variants builds each once. The key is the part, its overrides AND
    the state of the source -- without the last, an edited part silently serves the mesh built
    before the edit, and the sweep reports the old geometry's numbers."""
...
def mass_properties(stl, rho=PLA_RHO, rotate_x=0.0):
    ...
    `rotate_x` degrees about x is applied FIRST, and it is not a convenience: a part is
