...
def run(feed=0.0, block="teal", restitution=0.4, mu=0.35, seconds=3.0, overrides=None):
    ...
    core.static_mesh(
        core.build_part("spiral_ramp", overrides, obj=True),
        restitution=restitution,
        mu=mu,
    )
    col = p.createCollisionShape(
        p.GEOM_MESH,
        fileName=core.build_part(block, obj=True),
        meshScale=[core.MM] * 3,
    ...
    p.disconnect()
    return dict(
        vexit=vexit, looped=looped, reach=reach + SIDE / 2, through=reach > SIDE / 2
    )


def main():
    print(
        "270 deg loop off one corner: 60 deg side exit -> round the outside -> the low bore"
    )
    print(
        "loop radius %.0f, channel floor %.1f -> %.1f (%.1f mm of drop), bank %.0f deg\n"
    ...
    print(
        "out the far side, %%   n=%d per cell (%d bounce x %d friction)"
        % (n, len(BOUNCE), len(FRICTION))
    )
